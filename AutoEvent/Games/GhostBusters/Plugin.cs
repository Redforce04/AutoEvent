...
public class Plugin : Event, IInternalEvent, IEventSound
    {
        ...
        protected override void ProcessFrame()
        {
            ...
            foreach (Player player in Player.GetPlayers())
            {
                ...
                if (CurrentStage == Stage.Midnight)
                {
                    ...
                    {
                        player.SendBroadcast(Translation.GhostBustersMidnightGhostMessage.Replace("{time}", time), (ushort) FrameDelayInSeconds);
                    }
                    else
                    {
                        player.SendBroadcast(Translation.GhostBustersMidnightHunterMessage.Replace("{time}", time), (ushort) FrameDelayInSeconds);
                    }
                }
                else
                {
                    player.SendBroadcast(
                        Translation.GhostBustersRunning.Replace("{time}", time).Replace("{ghosts}", ghosts.ToString())
        ...
        protected override void OnFinished()
        {
            ...
            {
                if(ghosts > 0)
                    Map.Broadcast(10, Translation.GhostBustersTie);
                else
